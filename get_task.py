...
def get_task(args):
    ...
    if task_name == "mnli":
        ...
        if os.path.isfile(
            ...
            dataset = {
                key: pd.read_csv(data_path.format(key)).fillna("")
        ...
        all_train_text_to_encode = [
            (raw_item["commit_message"], raw_item["diff"])
            for raw_item in total_train_examples
        ]
        all_eval_text_to_encode = [
            (raw_item["commit_message"], raw_item["diff"])
        ...
        def format_example(example, label_map, **kwargs):
            return (
                f"Was this commit made to fix a vulnerability, reveal the presence of security issues in the code, or have security implications?\nCommit message: {example['commit_message']}\nCommit diff: {example['diff']}",
                f"Answer: {label_map[example['label']]}",
            )
        ...
        def group_file_to_commit(data):
            p_cve = data[0]
            p_cve_id = p_cve["cve_list"]
            p_repo = p_cve["repo"]
            p_c_id = p_cve["commit_id"]
            grouped_data = [[p_cve]]
            for i in range(1, len(data)):
                ...
                repo = cve["repo"]
                c_id = cve["commit_id"]
                if cve_id == p_cve_id and repo == p_repo and c_id == p_c_id:
                    grouped_data[-1].append(cve)
                else:
                    grouped_data.append([cve])
                    p_cve_id = cve_id
                    p_repo = repo
                    p_c_id = c_id
            return grouped_data
        ...
        if os.path.isfile(train_file) and os.path.isfile(test_file):
            ...
            data_path = os.path.join(args.data_cache_dir, "{}_set.json")
            dataset = {}
            for key in ["train", "validation", "test"]:
                with open(data_path.format(key), "r") as f:
                    dataset[key] = json.load(f)
                    for obj in dataset[key]:
                        if type(obj["msg"]) != str:
                            obj["msg"] = ""
                        if type(obj["user"]) != str:
                            obj["user"] = ""
            dataset["train"].extend(dataset["validation"])
            # print(dataset["train"][:3])
            total_train_examples = group_file_to_commit(dataset["train"])
            with open(train_file, "w") as f:
                json.dump(total_train_examples, f, indent=4)
            total_eval_examples = group_file_to_commit(dataset["test"])
            with open(test_file, "w") as f:
                json.dump(total_eval_examples, f, indent=4)

        all_train_text_to_encode = []
        for commit in total_train_examples:
            msg = commit[0]["msg"]
            diff = []
            for i in range(len(commit)):
                ...
                rem_diff = c["REM_DIFF"]
                add_diff = c["ADD_DIFF"]
                for j in range(len(rem_diff)):
                    diff.append(f"Removed: `{rem_diff[j]}` Added: `{add_diff[j]}`")
            diff = "\n".join(diff)
            all_train_text_to_encode.append((msg, diff))
        all_eval_text_to_encode = []
        for commit in total_eval_examples:
            msg = commit[0]["msg"]
            diff = []
            for i in range(len(commit)):
                c = commit[i]
